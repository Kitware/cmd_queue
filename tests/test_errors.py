def test_failures_on_each_backend():
    ...
    for backend in backends:
        ...
        self.submit('echo "job3 never runs"', depends=[job2])
        job4 = self.submit('echo "job4 passes" && true')
        job5 = self.submit('echo "job5 fails" && false', depends=[job4])
        self.submit('echo "job6 never runs"', depends=[job5])
        self.submit('echo "job7 never runs"', depends=[job4, job2])
        self.submit('echo "job8 never runs"', depends=[job4, job1])
        self.print_commands()
        self.run()
        self.read_state()
