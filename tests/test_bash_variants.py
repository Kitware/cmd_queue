...
def test_primary_bash_job_text_variants():
    ...
    for variant in main_variants:
        ...
        if variant['preamble']:
            job_kwargs['preamble'] = ['export SETUP_LINE1=1', 'export SETUP_LINE2=2']
        ...
        if variant['__testname__'] == 'the works':
            ...
            assert 'popd' in text
            assert 'CHDIR_OK' in text, "cwd=True should define CHDIR_OK and guard popd"
...
def test_bash_job_variants_syntax_grided():
    ...
    for variant in grid_variants:
        ...
        if proc.returncode == 0:
            ...
            raise AssertionError(f"bash syntax error: \nSTDERR:\n{proc.stderr}\nSCRIPT:\n{text}")
        ...
        if variant['log']:
            ...
            if hasattr(self, 'log_fpath'):
                assert str(self.log_fpath) in text, "log=True should reference log_fpath"
            if variant['with_gaurds']:
                assert 'set -o pipefail' in text, "log=True should enable pipefail"
                assert 'set +o pipefail' in text, "log=True should restore pipefail"
        else:
            # When log is disabled, we should not see pipefail boilerplate
            assert 'set -o pipefail' not in text, "log=False should not enable pipefail"
            assert 'set +o pipefail' not in text, "log=False should not restore pipefail"
        ...
        if variant['with_gaurds']:
            assert 'set +e' in text, "with_gaurds=True should disable exit-on-error"
            # We should enable xtrace somewhere (unless bookkeeper disables it; in your tests it should not)
            assert 'set -x' in text, "with_gaurds=True should enable command echo"
            # Return code capture should be hidden inside brace trick
            assert '{ RETURN_CODE=$?' in text, "with_gaurds=True should capture RETURN_CODE in brace trick"
            assert 'set +x -e' in text, "with_gaurds=True should disable echo and re-enable -e"
            # Ensure we don't have a noisy RETURN_CODE=$? line outside the brace trick
            bad_lines = [ln for ln in text.splitlines() if ln.strip().startswith('RETURN_CODE=$?')]
            ...
            assert 'set -x' not in text, "with_gaurds=False should not enable xtrace"
            assert 'set +x -e' not in text, "with_gaurds=False should not include brace trick toggles"
            assert '{ RETURN_CODE=$?' not in text, "with_gaurds=False should not include brace trick capture"

        # --- Status behavior
        if variant['with_status']:
            assert 'Mark job as running' in text, "with_status=True should mark job as running"
            assert 'Mark job as stopped' in text, "with_status=True should mark job as stopped"
            assert 'printf "pass" >' in text, "with_status=True should write pass marker"
            assert 'printf "fail" >' in text, "with_status=True should write fail marker"
            assert 'stat' in text or 'status' in text, "with_status=True should dump status JSON"
            ...
            assert 'printf "pass" >' not in text, "with_status=False should not write pass marker"
            assert 'printf "fail" >' not in text, "with_status=False should not write fail marker"
        ...
        if variant['depends'] and variant['with_status']:
            assert 'if [ -f' in text, "depends+with_status should emit dependency condition"
            assert 'RETURN_CODE=126' in text, "depends+with_status should set skip RETURN_CODE=126"
        else:
        ...
        if variant['cwd'] or variant['preamble']:
            ...
            if 'internal condition check' in text:
                assert 'fi  # internal condition check' in text, "internal if must be closed"
            else:
...
def test_bashjob_exec_depends_met_runs():
    with tempfile.TemporaryDirectory() as tmp_path:
        ...
        outfile = tmp_path / "ran.txt"
        job = BashJob(f'echo ran > "{outfile}"', name='job2', cwd=str(workdir), depends=[dep])
...
def test_bashjob_exec_cwd_missing_skips_command():
    with tempfile.TemporaryDirectory() as tmp_path:
        ...
        outfile = tmp_path / "ran.txt"
        job = BashJob(f'echo ran > "{outfile}"', name='job2', cwd=str(missing_dir))
        job.preamble = ['export SETUP_LINE1=1']
        job.log = False
